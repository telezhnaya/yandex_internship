...
class Person(object):
    ...
    def add_link(self, other, rel_type):
        if rel_type in ["child", "son", "daughter"]:
            ...
            # Теперь other имеет правильных родителей.
    ...
    def parent_request(self, gender):
        return ", ".join(self.get_parents(gender))

    def child_request(self, gender):
        return ", ".join(self.get_children(gender))
    ...
    def update_parents(self, new_dad, new_mom=None):
        if self.dad:
            if not self.dad.real:
                # Тогда избавимся от него и возьмем новых родителей
    ...
    def marry(self, wife):
        ...
        self.set_parent_to_children(wife)
        wife.set_parent_to_children(self)
        self.children.update(wife.children)
        wife.children = self.children
    ...
    def get_parents(self, gender=Gender.unknown):
        answer = []
    ...
    def get_children(self, gender=Gender.unknown):
...
ph.add("Carol is Darren's sister")
